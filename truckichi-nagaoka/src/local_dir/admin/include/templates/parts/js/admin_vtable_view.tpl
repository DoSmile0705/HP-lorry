...
var table;
	...
	editor = new $.fn.dataTable.Editor({
		...
		, idSrc:'table_info_id'
		, fields: [
			{ label: null, name:'visible_name'}
			, { label: null, name:'approvable_flg', type: "select"
				, options: [
					<!--HP{foreach from=$approvableFlgArray key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
			, { label: null, name:'server_side_flg', type: "select"
				, options: [
					<!--HP{foreach from=$serversideFlgArray key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
	...
	table = $("#jqlist").DataTable({
		...
		, columns: [
			...
			, {data: 'table_info_id'}
			, {data: 'table_name'}
			, {data: 'visible_name', className: 'editable'}
			, {data: 'table_type'}
			, {data: 'approvable_flg', className: 'editable',
				render: function (data, type, row){ return row.approvable_flg_name; }
			}
			, {data: 'server_side_flg', className: 'editable',
				render: function (data, type, row){ return row.server_side_flg_name; }
			}
			, {data: 'update_user'}
			, {data: 'update_time'}
		]
	...
	table.on('click', '.b_detail', function(){
		...
		location.href = url_edit + '?mode=detail_table&tid=' + table_info_id;
		return false;
	});
	// 編集ボタン
	table.on('click', '.b_edit', function(){
		...
		var data = table.row( tr ).data();
		var table_info_id = data['table_info_id'];
		location.href = url_edit + '?mode=edit_table&tid=' + table_info_id;
		return false;
	});
	// 削除ボタン
	table.on('click', '.b_delete', function(){
		...
		var data = table.row( tr ).data();
		var table_info_id = data['table_info_id'];
		deleteTableConfilm(url_edit, table_info_id);
	});
