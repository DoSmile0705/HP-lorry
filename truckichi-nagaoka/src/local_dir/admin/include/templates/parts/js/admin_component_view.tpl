...
var table;
	...
	// 一覧取得のurl
	var url_api = "<!--HP{$documentRoot}HP-->api/v1/components/";
	...
	editor = new $.fn.dataTable.Editor({
		...
		, idSrc:'component_id'
		, fields: [
			{label: null, name: "component_name"}
			, {label: null, name: "display_rows"}
			, {label: null, name: "top_panel_flg", type: "select"
				, options: [
					<!--HP{foreach from=$topPanelFlgArray key='key' item='row'}HP-->
	...
	table = $("#jqlist").DataTable({
		...
		, columns: [
			...
			// 1列目以降
			, {data: 'component_id'}
			, {data: 'component_name', className: 'editable'}
			, {data: 'component_type'}			// 編集不可
			, {data: 'display_rows', className: 'editable'}
			, {data: 'top_panel_flg', className: 'editable',
				render: function (val, type, row){ return row.top_panel_flg_name; }
			}
			, {data: 'update_user'}
			, {data: 'update_time'}
		]
		...
		//	$('input.editor-top_panel_flg', row).prop( 'checked', data.top_panel_flg == 1 );
		//}
		, responsive: true
	...
	// 詳細、編集のurl
	var url_edit = "<!--HP{$documentRoot}HP-->component.php";
	...
	table.on('click', '.b_detail', function(){
		...
		location.href = url_edit + '?mode=detail&component_id=' + component_id;
		return false;
	});
	// 編集ボタン
	table.on('click', '.b_edit', function(){
		...
		var data = table.row( tr ).data();
		var component_id = data['component_id'];
		location.href = url_edit + '?mode=edit&component_id=' + component_id;
		return false;
	});
	// 削除ボタン
	table.on('click', '.b_delete', function(){
		...
		var data = table.row( tr ).data();
		var component_id = data['component_id'];
		deleteComponentConfilm(url_edit, component_id);
	});
