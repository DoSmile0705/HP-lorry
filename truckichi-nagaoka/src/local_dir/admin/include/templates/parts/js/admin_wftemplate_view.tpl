...
var table;
	...
	// 一覧取得のurl
	var url_api = "<!--HP{$documentRoot}HP-->api/v1/wftemplates/";
	...
	editor = new $.fn.dataTable.Editor({
		...
		, table: "#jqlist"
		, idSrc:'wf_template_id'
		, fields: [
			{ label: null, name:'wf_template_name'}
			, { label: null, name:'mail_send', type: "select"
				, options: [
					<!--HP{foreach from=$mailSendFlg key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
			, { label: null, name:'group_id', type: "select"
				, options: [
					<!--HP{foreach from=$groupList key='key' item='val'}HP-->
					{ label: "<!--HP{$val.group_name|escape}HP-->", value: "<!--HP{$val.group_id|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
			, { label: null, name:'approval_flow_type', type: "select"
				, options: [
					<!--HP{foreach from=$approvalFlowType key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
	...
	table = $("#jqlist").DataTable({
		...
		, columns: [
			...
			// 1列目以降
			, {data: 'wf_template_id'}
			, {data: 'wf_template_name', className: 'editable'}
			, {data: 'mail_send', className: 'editable',
				render: function (data, type, row){ return row.mail_send_name; }
			}
			, {data: 'group_id', className: 'editable',
				render: function (data, type, row){ return row.group_name; }
			}
			, {data: 'approval_flow_type', className: 'editable',
				render: function (data, type, row){ return row.approval_flow_type_name; }
			}
			, {data: 'update_user'}
			, {data: 'update_time'}
		]
	...
	// 詳細、編集のurl
	var url_edit = "<!--HP{$documentRoot}HP-->wftemplate.php";

	// 詳細ボタン
	table.on('click', '.b_detail', function(){
		tr = $(this).closest('tr');
		var data = table.row( tr ).data();
		var wf_template_id = data['wf_template_id'];
		location.href = url_edit + '?mode=detail&tid=' + wf_template_id;
		return false;
	});
	// 編集ボタン
	table.on('click', '.b_edit', function(){
		...
		var data = table.row( tr ).data();
		var wf_template_id = data['wf_template_id'];
		location.href = url_edit + '?mode=edit&tid=' + wf_template_id;
		return false;
	});
	// 削除ボタン
	table.on('click', '.b_delete', function(){
		...
		var data = table.row( tr ).data();
		var wf_template_id = data['wf_template_id'];
		deleteWfTemplateConfilm(url_edit, wf_template_id);
	});
