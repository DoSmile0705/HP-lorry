...
var table;
	...
	editor = new $.fn.dataTable.Editor({
		...
		, idSrc:'profile_id'
		, fields: [
			{ label: null, name:'profile_name'}
			, { label: null, name:'profile_type', type: "select"
				, options: [
					<!--HP{foreach from=$profileType key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
			, { label: null, name:'mypage_editable_level', type: "select"
				, options: [
					<!--HP{foreach from=$mypageEditable key='key' item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$key|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
	...
	table = $("#jqlist").DataTable({
		...
		, columns: [
			...
			// 1列目以降
			, {data: 'profile_id'}
			, {data: 'profile_name', className: 'editable'}
			, {data: 'profile_type', className: 'editable',
				render: function (data, type, row){ return row.profile_type_name; }
			}
			, {data: 'mypage_editable_level', className: 'editable',
				render: function (val, type, row){ return row.mypage_editable_level_name; }
			}
			, {data: 'update_user'}
			, {data: 'update_time'}
		]
	...
	// 詳細、編集のurl
	var url_edit = "<!--HP{$documentRoot}HP-->profile.php";
	...
	table.on('click', '.b_detail', function(){
		...
		location.href = url_edit + '?mode=detail&pid=' + profile_id;
		return false;
	});
	// 編集ボタン
	table.on('click', '.b_edit', function(){
		...
		var data = table.row( tr ).data();
		var profile_id = data['profile_id'];
		location.href = url_edit + '?mode=edit&pid=' + profile_id;
		return false;
	});
	// 削除ボタン
	table.on('click', '.b_delete', function(){
		...
		var data = table.row( tr ).data();
		var profile_id = data['profile_id'];
		deleteProfileConfilm(url_edit, profile_id);
	});
