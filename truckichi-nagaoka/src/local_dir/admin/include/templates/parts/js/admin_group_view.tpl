...
var table;
	...
	// 一覧取得のurl
	var url_api = "<!--HP{$documentRoot}HP-->/api/v1/groups/";
	...
	editor = new $.fn.dataTable.Editor({
		...
		, idSrc:'group_id'
		, fields: [
			{ label: null, name:'group_name'}
			, { label: null, name:'theme_color', type: "select"
				, options: [
					<!--HP{foreach from=$themeColor item='row'}HP-->
					{ label: "<!--HP{$row|escape}HP-->", value: "<!--HP{$row|escape}HP-->" },
					<!--HP{/foreach}HP-->
				]
			}
		]
	...
	table = $("#jqlist").DataTable({
		...
		, columns: [
			...
			// 1列目以降
			, {data: 'group_id'}
			, {data: 'group_name', className: 'editable'}
			, {data: 'upper_group_id'}			// 編集不可
			, {data: 'theme_color', className: 'editable'}
			, {data: 'update_user'}
			, {data: 'update_time'}
		]
		, responsive: true
	...
	// 詳細、編集のurl
	var url_edit = "<!--HP{$documentRoot}HP-->group.php";
	...
	table.on('click', '.b_detail', function(){
		...
		location.href = url_edit + '?mode=detail&group_id=' + group_id;
		return false;
	});
	// 編集ボタン
	table.on('click', '.b_edit', function(){
		...
		var data = table.row( tr ).data();
		var group_id = data['group_id'];
		location.href = url_edit + '?mode=edit&group_id=' + group_id;
		return false;
	});
	// 削除ボタン
	table.on('click', '.b_delete', function(){
		...
		var data = table.row( tr ).data();
		var group_id = data['group_id'];
		deleteGroupConfilm(url_edit, group_id);
	});
